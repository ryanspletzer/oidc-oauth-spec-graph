...
- **RFC 10027** - Best Current Practice for Security of Cross-Device Flows: Describes threats against OAuth cross-device authorization and session-transfer flows (such as the Device Authorization Grant and CIBA), notably social-engineering attacks that exploit unauthenticated channels, and the mitigations implementers should apply
...
- **Transaction Tokens (Draft)**: Defines short-lived, signed JWT Transaction Tokens that propagate user identity, workload identity, and authorization context across a call chain of workloads within a trusted domain, issued via an OAuth 2.0 Token Exchange profile
- **AAuth - Agentic Authorization OAuth 2.1 Extension (Draft)**: Defines the Agent Authorization Grant, an OAuth 2.1 extension letting AI agents that interact with users over PSTN/texting channels (where redirect-based OAuth flows are impossible) obtain scoped access tokens using information collected via natural-language conversation
