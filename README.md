...
- **Identity Assertion Authorization Grant (Draft)**: Enables applications to obtain OAuth 2.0 access tokens by leveraging identity assertions (such as OpenID Connect ID Tokens or SAML assertions) through a coordinated enterprise identity provider, combining Token Exchange with JWT-based authorization grants to facilitate cross-application access management
